...
void SoundGenerator::init()
{
  if (!m_initDone) {
    ...
    //REMOVEDif (m_genMethod == SoundGenMethod::Auto)
    //REMOVED  m_genMethod = CurrentVideoMode::get() == VideoMode::CVBS ? SoundGenMethod::SigmaDelta : SoundGenMethod::DAC;
    if (m_gpio == GPIO_AUTO)
      m_gpio = GPIO_NUM_25;  //Fixed to PIN 25 for TTGO CGA32 1.4
